...
# MIGRAÇÃO DE JOBS
# -------------------------------
'''def try_migrate(job, origem_idx, workers_meta, worker_queues):
    """Tenta migrar job para o servidor com maior capacidade livre."""
    dest_idx = find_best_worker_load(workers_meta)

    # não há servidor com capacidade disponível
    if dest_idx is None or dest_idx == origem_idx:
        return False

    origem = workers_meta[origem_idx]
    destino = workers_meta[dest_idx]

    # migra se o destino estiver mais livre que a origem
    if (destino["capacidade"] - destino["carga_atual"]) > (origem["capacidade"] - origem["carga_atual"]):
        print(timestamp(),
              f"* MIGRAÇÃO: Job {job['id']} de Servidor {origem['id']} → Servidor {destino['id']}", flush=True)

        origem["carga_atual"] -= 1
        destino["carga_atual"] += 1

        worker_queues[dest_idx].put(job)
        return True
...
def main(input_path):
    ...
    print(timestamp(), "Simulação iniciada - política: SJF")
    ...
    while True:
        ...
        while not result_queue.empty():
            ...
            if msg["event"] == "start":
                jid = msg["job_id"]
                wid = msg["worker_id"]
                started_jobs.add(jid)
                st = msg.get("start_time", now())

                worker_last_start[wid] = st
                job_start_times[jid] = st
                continue
            ...
            if msg["event"] == "finish":
                ...
                workers_meta[idx]["jobs_em_execucao"] = [
                    j for j in workers_meta[idx]["jobs_em_execucao"] if j["id"] != jid
                ]
                ...
                wait_time = start_time - chegada if chegada is not None else 0.0

                metrics["completed"] += 1
                metrics["sum_response_time"] += response_time
                metrics["max_wait"] = max(metrics["max_wait"], wait_time)

                print(timestamp(),
                      f"Servidor {wid} concluiu Job {jid} (resp={response_time:.2f}s)", flush=True)

        # -------------------------
        # MIGRAÇÃO DINÂMICA
        # -------------------------
        '''for origem_idx, w in enumerate(workers_meta):
            # iteramos sobre a cópia para poder remover
            for job in w["jobs_em_execucao"][:]:
                jid = job["id"]
                # só migrar se o job ainda NÃO tiver começado no worker
                if jid in started_jobs:
                    continue

                migrated = try_migrate(job, origem_idx, workers_meta, worker_queues)
                if migrated:
                    # se migrou, atualizar lista local (origem) e job_start_times para novo momento
                    w["jobs_em_execucao"].remove(job)
                    job_start_times[jid] = now() '''


        done = (
                arrival_i >= len(arrivals)
                and not pending
                and all(w["carga_atual"] == 0 for w in workers_meta)
        )
        if done:
            break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simulador Orquestrador SJF")
